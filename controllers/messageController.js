  ...
  try {
    ...
    if (!existingConversation) {
      return res.status(500).json({
        success: false,
        message: "No conversation found against that Id",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to create conversation or message" });
  }
};
...
exports.getMessages = async (req, res) => {
  try {
    ...
    const otherId = existingConversation.participants.filter(
    ...
    if (messages.length > 0) {
      ...
      return res
        .status(200)
        .json({ success: true, messages, user: otherId[0] });
    }
    return res
      .status(200)
      .json({ success: false, messages: [], user: otherId[0] });
  } catch (error) {
    // If an error occurs during the execution, respond with a 500 Internal Server Error
    res.status(500).json({ message: "Failed to fetch conversations" });
  }
...
exports.getMessageConversationById = async (req, res) => {
  try {
