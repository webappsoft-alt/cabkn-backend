...
router.post("/admin", async (req, res) => {
  ...
  if (user.type !== "admin")
    return res
      .status(400)
      .send({ success: false, message: "Invalid credentials" });
  await User.findOneAndUpdate({ _id: user._id }, { fcmtoken }, { new: true });
...
router.post("/:type?", async (req, res) => {
  if (req.params.type == "social-login") {
    ...
    if (!["customer", "rider"].includes(type))
      return res
        .status(400)
        .send({ success: false, message: "Invalid user type." });

    const user = await User.findOne({ email: lowerCaseEmail, type });

    if (["rider"].includes(type) && !user)
